...
bioclean = lambda t: re.sub('[.,?;*!%^&_+():-\[\]{}]', '',
                            t.replace('"', '').replace('/', ' ').replace('\\', '').replace("'",'').strip().lower()).split()
...
def clean_dictionary(phrase_dic): # note disable cleaning for now - so that the keys still match with graphex keysc
...
def create_pooling_weights_for_batch(idx_u, phrase_dic, batch):
    counts = []
    for idx in idx_u:
        counts.append(Counter(idx.cpu().numpy().tolist()))
    json_list = []
    for idx, phr_id in enumerate(batch):
        freqs = counts[idx]
