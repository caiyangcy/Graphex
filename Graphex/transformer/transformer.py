...
from transformer.transformer_utils import share_embedding, LabelSmoothing, NoamOpt, _get_attn_subsequent_mask, get_input_from_batch, get_output_from_batch, get_text_from_batch
...
class Generator(nn.Module):
    ...
    def forward(self, x, attn_dist=None, enc_batch_extend_vocab=None, extra_zeros=None, temp=1, beam_search=False):
        ...
        if config.pointer_gen and extra_zeros is not None:
            ...
            if beam_search:
                extra_zeros = extra_zeros.repeat(config.beam_size, 1, 1)[:vocab_dist_.size(0),:,:]
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab, model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.decoder = Decoder(config.emb_dim, config.hidden_dim, num_layers=config.hop, num_heads=config.heads, 
                                total_key_depth=config.depth, total_value_depth=config.depth, max_length=config.max_enc_steps,
                                filter_size=config.filter) #,universal=config.universal)
    ...
    def train_one_batch(self, batch, data_loader_all=None, train=True, log=False):
        ## pad and other stuff
        enc_batch, _, _, enc_batch_extend_vocab, extra_zeros, _, _, _ = get_input_from_batch(batch)
        dec_batch, _, _, _, target_batch, _ = get_output_from_batch(batch)
        source, target = get_text_from_batch(batch)
        ...
        # print(f" tgt mask shape: {tgt_mask.shape}; mask shape: {mask.shape} ")
        ...
        if log:
            ...
            for i in range(len(source)):
                f.write(" ".join(source[i]))
                f.write("\n")
                f.write(str(target[i]))
                f.write("\n")
                for j in range(len(attn_dist[i])):
                    for k in range(len(attn_dist[i][j])):
                        f.write(str(attn_dist[i][j][k].item()))
                        f.write(" ")
                    f.write("\n")
        ## compute output dist
        ...
        loss = self.criterion(logit.contiguous().view(-1, logit.size(-1)), target_batch.contiguous().view(-1))
        ...
        if(train):
            ...
            self.optimizer.step()
        if(config.label_smoothing): 
            loss = self.criterion_ppl(logit.contiguous().view(-1, logit.size(-1)), target_batch.contiguous().view(-1))
