...
from graphex.graphex import Graphex
from utils.eval import evaluate_graphex as evaluate
...
for di in file_list[:1]:
    ...
    loss,ppl,bleu_score_b = evaluate(model,data_loader_test,model_name=config.model,ty='test',verbose=False,log=True, 
                        result_file="results/" + di + "/results_graph.txt", ref_file="results/" + di + "/ref_graph.txt", case_file="results/" + di + "/case_graph.txt")
