...
class Dataset(data.Dataset):
    ...
    def __init__(self, data, node_dic, vocab, per=0):
...
class Data:
    ...
    def get_all_data(self,batch_size):
        ...
        dataset_train = Dataset(tr, self.node_dic, self.vocab)
        dataset_total = Dataset(total, self.node_dic, self.vocab)
        ...
        dataset_val = Dataset(val, self.node_dic, self.vocab)
        data_loader_val = torch.utils.data.DataLoader(dataset=dataset_val,
                                                batch_size=batch_size,
                                                shuffle=False,collate_fn=collate_fn)  

        dataset_test = Dataset(test, self.node_dic, self.vocab)
