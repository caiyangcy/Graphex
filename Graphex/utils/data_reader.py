    ...
    def __init__(self, data, node_dic, node_emb, node_emb2, vocab, per=0):
        ...
        for d in data:
            ...
            if not os.path.exists(config.data_dir + "/node_embeddings_phrases.p"):
                ...
                name_clean = " ".join( d[0].lower().split() )
                self.node_emb.append(node_emb[ name_clean ])

            if not os.path.exists(config.data_dir + "/node_embeddings_phrases_def.p"):
                self.node_emb2.append(0)
            else:
                name_clean = " ".join( d[1].lower().split() )
                self.node_emb2.append(node_emb2[ name_clean ])
...
def prepare_data_seq():
    ...
    test_def = open(config.data_dir + "/test_def_gen.txt", "r").readlines()
    ...
    print("return from node_embeddings_phrases")
...
class Data:
    def __init__(self):
        ...
        # print("node dic: ", self.node_dic)
        # print("node emb size: ", self.node_emb2 )

        # print("\nnode dic keys: ", self.node_dic.keys() )
        # print("\nnode emb keys: ", self.node_emb2.keys() )
        
        
        # print("overlap between node dic and emb keys: ",     len(     set(list(self.node_dic.keys())) & set(list(self.node_emb2.keys()))     )    )
        # print("overlap in node dic : ",     len(     set(list(self.node_dic.keys())) - set(list(self.node_emb2.keys()))     )    )
        # print("overlap in emb keys: ",     len(     set(list(self.node_emb2.keys()))  - set(list(self.node_dic.keys()))    )    )

        # print("\noverlap in node dic : ",    set(list(self.node_dic.keys())) - set(list(self.node_emb2.keys()))       )
        # print("\noverlap in emb keys: ",     set(list(self.node_emb2.keys()))  - set(list(self.node_dic.keys()))      )
