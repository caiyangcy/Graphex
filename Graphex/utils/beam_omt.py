...
from transformer.transformer_utils import _get_attn_subsequent_mask
...
class Translator(object):
    ...
    def translate_batch(self, src_seq, data_loader_all=None):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
                ## masking
                tgt_mask = dec_seq.data.eq(config.PAD_idx).unsqueeze(1)
                mask_src = torch.cat([mask_src[0].unsqueeze(0)]*tgt_mask.size(0),0)
                ...
                dec_output, attn_dist = self.model.decoder(self.model.embedding(dec_seq), enc_output, mask_src, dec_mask )
        ...
        with torch.no_grad():
            #-- Encode
            enc_batch, enc_padding_mask, enc_lens, enc_batch_extend_vocab, extra_zeros, c_t_1, coverage, node_emb, node_emb2 = get_input_from_batch(src_seq)
            enc_batch=enc_batch[:,:1000]
            ...
            src_enc = self.model.encoder(torch.cat((self.model.embedding(enc_batch), node_embedding, node_embedding_def), 2),mask_src)
...
def get_input_from_batch(batch):
    ...
    return enc_batch, enc_padding_mask, enc_lens, enc_batch_extend_vocab, extra_zeros, c_t_1, coverage, node_emb, node_emb2
