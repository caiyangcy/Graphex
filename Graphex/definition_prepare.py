...
from utils.data_reader_transformer_only import Data
from transformer.transformer import Transformer
from utils.eval import evaluate_transformer as evaluate
...
p = Data()
_, data_loader_train, data_loader_val, _ = p.get_all_data(batch_size=config.batch_size)
...
for di in file_list:
    ...
    p = Data()
    tmp = p.get_all_data(batch_size=config.batch_size)

    _, _, _, data_loader_test = p.get_all_data(batch_size=config.batch_size)
    ...
    loss,ppl,bleu_score_b = evaluate(model, data_loader_test, model_name=config.model, ty='test', verbose=False, log=True, result_file="data/" + di + "/test_def_gen.txt", 
                                     ref_file="results/" + di + "/ref_transformer_full.txt", case_file="results/" + di + "/case_transformer_full.txt")
